...
const MainLayout = ({ children }) => {
  ...
  return (
    <div className={`flex min-h-screen relative overflow-x-hidden transition-colors duration-500 ${isDark ? 'bg-[#050810] text-slate-100' : 'bg-[#FAFAFF] text-slate-900'}`}>
      {/* Global Cute Glows */}
      ...
      <main
        className={`flex-1 min-w-0 max-w-full relative z-10 min-h-screen overflow-y-auto transition-all duration-300 ${isImmersiveRoute ? 'p-0' : 'px-4 md:px-6 lg:px-10 py-10'} ${isDark ? (isCelestialRoute ? 'mesh-bg' : 'bg-[#050810]') : 'bg-white'}`}
      >

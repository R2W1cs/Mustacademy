...
export default function TopicPodcastPlayer({ topic }) {
    ...
    useEffect(() => {
        ...
        // Logic for browser fallback if cloud fails
        const speakWithBrowser = (text, type) => {
            synth.cancel();
            ...
            const selectedVoice = findBestVoice(type);
            if (selectedVoice) {
                console.log(`[TTS] ${type} selected voice:`, selectedVoice.name);

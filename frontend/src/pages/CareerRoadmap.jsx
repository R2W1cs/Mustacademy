...
const IMPORTANCE_STYLE = {
    ...
    bonus:    "bg-white/5 text-white/60 border-white/10",
};
const PRIORITY_BADGE = {
    ...
    OPTIONAL:    "bg-white/5 text-white/55 border-white/8",
};
...
const SL = ({ icon: Icon, label }) => (
    <div className="flex items-center gap-2 mb-4">
        ...
        <p className="text-xs font-black uppercase tracking-[0.25em] text-white/55">{label}</p>
    </div>
);
    ...
    return (
        <div className="min-h-screen bg-[#070b14] text-white overflow-y-auto">
            ...
            <div className="relative z-10 max-w-[1200px] mx-auto px-6 md:px-10 py-12">
                <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-white/55 hover:text-white/65 transition-colors text-xs font-bold uppercase tracking-widest mb-14">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
                    <p className="text-xs font-black uppercase tracking-[0.35em] text-white/50 text-center mb-5">Select your target career</p>
                        ...
                        {CAREERS.map((career, i) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p className="text-sm font-black text-white/88 leading-tight">{career.title}</p>
                                    ...
                                    <div className="flex flex-wrap gap-1 mt-auto">{career.tags.map(t => <span key={t} className="text-sm text-white/50 font-mono">{t}</span>)}</div>
                                </motion.button>
                            );
                        })}
                    </div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="max-w-xl mx-auto">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-xs text-white/50 uppercase tracking-widest font-bold">or type your own career</span>
                        <div className="flex-1 h-px bg-white/5" />
                    </div>
                    <div className="flex gap-2">
                        <div className="relative flex-1">
                            <Search size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/50" />
...
const LoadingView = ({ step }) => (
    <div className="min-h-screen bg-[#070b14] flex items-center justify-center text-white">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative z-10 text-center max-w-xs w-full px-6">
            ...
            <p className="text-xs text-white/60 mb-8">Building a real 36-month plan with courses and projects</p>
            <div className="space-y-2.5 text-left">
                {LOADING_STEPS.map((s, i) => (
                    <div key={i} className="flex items-center gap-2.5">
                        ...
                        <span className={"text-sm transition-colors " + (i <= step ? "text-white/80" : "text-white/50")}>{s}</span>
                    </div>
                ))}
            </div>
        </motion.div>
    ...
    return (
        <div className="space-y-8 text-white">
            <div>
                ...
                <p className="text-sm text-white/70 leading-relaxed max-w-2xl">{phase.description}</p>
                {phase.why_this_order && (
                    <div className="mt-3 flex items-start gap-2 p-3 rounded-xl bg-indigo-500/5 border border-indigo-500/12">
                        ...
                        <p className="text-sm text-indigo-300/60 italic leading-relaxed">{phase.why_this_order}</p>
                    </div>
                )}
            </div>
                        ...
                        {phase.skills.map((skill, i) => (
                            <div key={i} className="p-4 rounded-2xl bg-white/[0.025] border border-white/5 hover:border-white/8 transition-colors">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {skill.subtopics?.map((sub, j) => <Tag key={j} className="bg-white/[0.04] border-white/6 text-white/65">{sub}</Tag>)}
                                </div>
                            </div>
                        ))}
                    </div>
                            ...
                            <div key={i} className="p-4 rounded-2xl bg-white/[0.025] border border-white/5 hover:border-white/10 transition-all">
                                <div className="flex items-start justify-between gap-3 mb-2">
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                        ...
                                        <span className="text-xs text-white/60 font-mono">{course.rating?.toFixed(1)}</span>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 flex-wrap mb-2">
                                    <span className="text-sm text-white/60">{course.platform}</span>
                                    <span className={"text-sm font-bold px-1.5 py-0.5 rounded-full " + (course.free ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-white/55")}>{course.free ? "FREE" : "PAID"}</span>
                                    <span className="flex items-center gap-1 text-xs text-white/55"><Clock size={9} />{course.hours}h</span>
                                </div>
                                {course.why && <p className="text-sm text-white/55 italic mb-2 leading-relaxed">{course.why}</p>}
                                {course.output && (
                                    <div className="flex items-start gap-1.5 mt-2 pt-2 border-t border-white/5">
                                        ...
                                        <p className="text-sm text-emerald-400/60">Output: {course.output}</p>
                                    </div>
                                )}
                            </div>
                        ))}
                    ...
                    <div className="space-y-3">
                        {phase.projects.map((proj, i) => (
                            <div key={i} className="p-4 rounded-2xl bg-emerald-500/[0.04] border border-emerald-500/12 hover:border-emerald-500/22 transition-colors">
                                ...
                                {proj.why && <p className="text-xs text-emerald-400/42 italic">Portfolio: {proj.why}</p>}
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
                {phase.tools?.length > 0 && (
                    <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                        ...
                        <div className="flex flex-wrap gap-2">{phase.tools.map((t, i) => <Tag key={i} className="bg-white/[0.04] border-white/7 text-white/70">{t}</Tag>)}</div>
                    </div>
                )}
                {phase.checkpoint && (
                    <div className="p-4 rounded-2xl bg-indigo-500/[0.04] border border-indigo-500/12">
                        <SL icon={ListChecks} label="Mastery Checkpoint" />
                        <p className="text-sm text-indigo-300/52 italic leading-relaxed">{phase.checkpoint}</p>
                    </div>
                )}
            </div>
...
const OverviewSection = ({ roadmap, onNavigate }) => {
    ...
    return (
        <div className="space-y-6 text-white">
            <div>
                <p className="text-xs font-black uppercase tracking-[0.3em] text-indigo-400 mb-2">3-Year Career Roadmap</p>
                <h1 className="text-4xl font-black mb-2 tracking-tight">{roadmap.career}</h1>
                <p className="text-sm text-white/65 italic">{roadmap.tagline}</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
                {[
                    ...
                    <div key={i} className="p-4 rounded-2xl bg-white/[0.025] border border-white/5">
                        ...
                        <p className="text-sm text-white/55 uppercase tracking-widest font-bold mb-1">{s.label}</p>
                        <p className={"text-sm font-black " + s.c}>{s.val || "—"}</p>
                    </div>
                ))}
            </div>
            <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                <p className="text-xs font-black uppercase tracking-widest text-white/55 mb-2">About This Role</p>
                <p className="text-sm text-white/70 leading-relaxed">{ov.description}</p>
            </div>
            <div className="grid md:grid-cols-2 gap-3">
                <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                    <p className="text-xs font-black uppercase tracking-widest text-white/55 mb-3">Top Hiring Companies</p>
                    <div className="flex flex-wrap gap-2">{ov.top_companies?.map((co, i) => <span key={i} className="text-xs px-3 py-1 rounded-full bg-white/5 border border-white/7 text-white/75 font-medium">{co}</span>)}</div>
                </div>
                <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                    <p className="text-xs font-black uppercase tracking-widest text-white/55 mb-3">Core Languages</p>
            ...
            <div>
                <p className="text-xs font-black uppercase tracking-widest text-white/55 mb-3">Jump to a Year</p>
                    ...
                    {years.map((yr, i) => {
                        ...
                        return (
                            <button key={i} onClick={() => onNavigate("year-" + i)} className={"p-4 rounded-2xl " + yc.bg + " border " + yc.ring + " text-left hover:opacity-80 transition-opacity"}>
                                ...
                                <p className="text-xs text-white/55 mt-1">{yr.phases?.length || 0} phases</p>
                            </button>
                        );
                    })}
                </div>
...
const StackTimeline = ({ timeline }) => (
    <div className="space-y-6 text-white">
        <div>
            <p className="text-xs font-black uppercase tracking-[0.3em] text-indigo-400 mb-1">Tools You Will Gain</p>
            <h2 className="text-3xl font-black mb-2">Tech Stack Timeline</h2>
            <p className="text-sm text-white/65">Technologies progressively added across 3 years.</p>
        </div>
        {timeline?.length ? (
        ...
        ) : <p className="text-white/60 text-sm">No timeline data generated.</p>}
    </div>
);

const InterviewPrep = ({ prep }) => {
    if (!prep) return <p className="text-white/60 text-sm">No interview prep data.</p>;
    return (
        <div className="space-y-6 text-white">
            <div>
                <p className="text-xs font-black uppercase tracking-[0.3em] text-amber-400 mb-1">Job Readiness</p>
                <h2 className="text-3xl font-black mb-2">Interview Preparation</h2>
                <p className="text-sm text-white/65">Start: {prep.start_at} · Duration: {prep.duration}</p>
            </div>
            {prep.areas?.length > 0 && (
                <div>
                    <p className="text-xs font-black uppercase tracking-widest text-white/55 mb-3">Focus Areas</p>
                    <div className="grid grid-cols-2 gap-2">
                        {prep.areas.map((area, i) => (
                            <div key={i} className="flex items-center gap-2 p-3 rounded-xl bg-white/[0.025] border border-white/5">
                                ...
                                <span className="text-xs text-white/75 font-medium">{area}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {prep.resources?.length > 0 && (
                <div>
                    <p className="text-xs font-black uppercase tracking-widest text-white/55 mb-3">Key Resources</p>
                    <div className="space-y-2">
                        {prep.resources.map((res, i) => (
                            <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.025] border border-white/5">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-white/55">{res.platform}</p>
                                    {res.why && <p className="text-xs text-white/55 italic mt-1">{res.why}</p>}
                                </div>
                            </div>
                        ))}
                    </div>
...
const ResourcesSection = ({ resources }) => {
    if (!resources?.length) return <p className="text-white/60 text-sm">No resources data.</p>;
    return (
        <div className="space-y-6 text-white">
            <div>
                <p className="text-xs font-black uppercase tracking-[0.3em] text-indigo-400 mb-1">Curated Must-Haves</p>
                <h2 className="text-3xl font-black mb-2">Essential Resources</h2>
                <p className="text-sm text-white/65">Books, channels, and platforms recommended by engineers in this field.</p>
            </div>
                        ...
                        <div className="space-y-4">
                            {res.items?.map((item, j) => (
                                <div key={j} className="pl-3 border-l-2 border-white/5">
                                    ...
                                    <p className="text-xs text-white/24">{item.by}</p>
                                    {item.why && <p className="text-xs text-white/55 italic mt-0.5 leading-relaxed">{item.why}</p>}
                                </div>
                            ))}
                        </div>
                    </div>
        ...
        <div className="w-full h-full bg-[#070b14] text-white flex flex-col overflow-hidden">
            <div className="h-11 shrink-0 flex items-center justify-between px-5 border-b border-white/5 bg-[#06090f]/95 backdrop-blur-xl z-10">
                <div className="flex items-center gap-3">
                    <button onClick={onBack} className="flex items-center gap-1.5 text-white/55 hover:text-white/65 transition-colors text-xs font-bold uppercase tracking-widest">
                        <ArrowLeft size={11} /> Careers
                    </button>
                    <div className="w-px h-3.5 bg-white/7" />
                    <span className="text-sm font-black text-white/75">{roadmap.career}</span>
                    ...
                    {roadmap.overview?.salary_range && (
                        <span className="hidden md:inline text-xs text-white/55 font-mono">{roadmap.overview.salary_range}</span>
                    )}
                </div>
                <button onClick={onRegenerate} className="flex items-center gap-1.5 text-white/50 hover:text-white/50 transition-colors text-xs font-bold uppercase tracking-widest">
                    ...
                    {navItems.map(item => {
                            ...
                            return (
                                <button key={item.id} onClick={() => toggleYear(item.yi)}
                                    ...
                                    <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        <span className="text-xs font-black text-white/75 group-hover:text-white/78 transition-colors leading-tight truncate">{item.label}</span>
                                    </div>
                                    <ChevronDown size={10} className={"text-white/50 shrink-0 transition-transform " + (openYears[item.yi] ? "rotate-180" : "")} />
                                </button>
                            );
                        }
                        if (item.kind === "phase") {
                            ...
                            return (
                                <button key={item.id} onClick={() => go(item.id)}
                                    ...
                                    <div className="min-w-0">
                                        <p className={"text-xs font-bold leading-tight truncate " + (isActive ? "text-white/88" : "text-white/65")}>{item.label}</p>
                                        <p className="text-sm text-white/50 font-mono">{item.sub}</p>
                                    </div>
                                </button>
                            );
                        }
                        return (
                            <button key={item.id} onClick={() => go(item.id)}
                                ...
                                <item.Icon size={12} className={isActive ? "text-indigo-400" : "text-white/50"} />
                                <span className={"text-sm font-bold " + (isActive ? "text-white/88" : "text-white/65")}>{item.label}</span>
                            </button>
                        );
                    })}
                </nav>
                            ...
                            {activeId.startsWith("year-") && !currentPhase && (() => {
                                ...
                                return (
                                    <div className="text-white">
                                        ...
                                        <p className="text-sm text-white/70 mb-8">{yr.theme}</p>
                                        <div className="space-y-3">
                                            {yr.phases?.map((ph, pi) => {
                                                ...
                                                return (
                                                    <button key={pi} onClick={() => { setOpenYears(p => ({ ...p, [yi]: true })); go(pid); }}
                                                        ...
                                                        <div className="flex items-center gap-2 text-xs text-white/55 shrink-0">

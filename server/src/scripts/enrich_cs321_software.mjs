/**
 * Enrich CS 321 Principles of Software Engineering topics with lesson markdown.
 * Source of truth: server/src/scripts/cs321/topics.mjs
 *
 * Usage: node src/scripts/enrich_cs321_software.mjs  (from server/)
 */
import dotenv from "dotenv";
import path from "path";
import { fileURLToPath } from "url";
import pool from "../config/db.js";
import { topics } from "./cs321/topics.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
dotenv.config({ path: path.join(__dirname, "../../.env") });

const FALLBACK_COURSE_ID = 174;
...
async function resolveCourseId() {
  const res = await pool.query(
    `SELECT id, name FROM courses
     WHERE name ILIKE '%CS 321%' OR name ILIKE '%Software Engineering%'
     ORDER BY CASE WHEN name ILIKE '%CS 321%' THEN 0 ELSE 1 END
     LIMIT 1`
  );
  ...
  console.warn(
    `  No CS 321 / Software Engineering course found; falling back to course_id=${FALLBACK_COURSE_ID}`
  );
...
async function enrich() {
  ...
  try {
    ...
    console.log(`Enriching CS 321 topics (course: ${course.name}, id=${courseId})...`);
    console.log(`  ${topics.length} lessons from cs321/topics.mjs`);
